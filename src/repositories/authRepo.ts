...
import {RefreshTokenModel} from "../types/model/refreshTokenModel";

class AuthRepo {
    public static async findUserByEmailOrUsername(emailOrUsername: string): Promise<User | null> {
        const result:QueryResult<UserModel> = await pool.query(
            "SELECT * FROM users WHERE users.email = $1 OR users.username = $1",
            [emailOrUsername]
        );
        if (result.rows.length === 0 || !result || !result.rows[0]) {
            return null;
        }

        const row: UserModel = result.rows[0];
        return {
            id: row.id,
            email: row.email,
    ...
    public static async getUserInfo (emailOrUsername: string): Promise<User | null> {
        const result: QueryResult<UserModel> = await pool.query(
    ...
    public static async logoutUser(token: string): Promise<boolean> {
        try {
            await pool.query(
    ...
    public static async saveRefreshToken(id: string, refreshToken: string, expiration: Date, userAgent: string | null, ipAddress: string | null): Promise<boolean> {
        try {
            await pool.query(
    ...
    public static async revokeRefreshToken(refreshToken: string): Promise<boolean> {
        let result: QueryResult<RefreshTokenModel>;
        try {
    ...
    public static async findRefreshToken(refreshToken: string) {
        try {
            const result: QueryResult<RefreshTokenModel> = await pool.query(
            ...
            const row: RefreshTokenModel = result.rows[0];
            return {
                userId: row.user_id,
    ...
    public static async searchOAuthUser(provider: string, providerId: string): Promise<string | null> {
        try {
    ...
    public static async createOAuthUser(user: {
        ...
        const userResult: QueryResult<UserModel> = await pool.query(
        ...
        if (userResult.rows.length === 0 || !userResult || !userResult.rows[0]) {
        ...
        return { id: userId, email: userResult.rows[0].email, username: userResult.rows[0].username } as User;
    }
}
